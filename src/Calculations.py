...
class Calculations:
    ...
    def calculateHypothenuse(self, x):
        ...
        for y in range(0, getmm_array.shape[0]):
            getmm_array[y][0] = getmm_array[y][0] / \
                math.sin(np.deg2rad(12))  # 12 veranderen naar angle

        return getmm_array

    def cartesianToSpherical(self, hypo_array, x):
        # 90-angle of angle
        polarangle = (.5 * math.pi) - \
            math.atan(hypo_array[y][1] / hypo_array[y][0])
    ...
    def run(self):
        ...
        for x in range(73):  # 73 later nog veranderen in rotations
            ...
            for y in range(0, hypo_array.shape[0]):
                polarangle, radius, motorAngle = self.cartesianToSpherical(
                    hypo_array, x)
                xcoordinate, ycoordinate, zcoordinate = self.SphericalToCartesian(
                    polarangle, radius, motorAngle)
